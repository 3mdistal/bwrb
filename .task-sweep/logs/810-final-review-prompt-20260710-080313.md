You are performing the final diff-only readiness review for Bowerbird issue #810.

Issue contract:
- Root completion matches every visible built CLI command, including init, while excluding hidden open/search.
- Schema and template subcommand candidates match built help; nonexistent template show is removed.
- Template detail-name completion uses the shipped template list <type> <name> surface.
- Recent option completion includes and exactly contract-checks the built command flags plus supported global flags.
- Canonical completion docs match runtime behavior.

Prior review found no blockers and suggested making the recent option contract bidirectional. This final diff includes that stronger exact-set test.
