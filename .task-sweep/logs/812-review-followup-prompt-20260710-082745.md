You are performing the final follow-up readiness review for Bowerbird issue #812.

The previous exact-model review returned NON-BLOCKING with these substantive notes:
1. Drop the synthetic empty context line created by splitting a newline-terminated file.
2. Avoid eagerly reading every vault candidate file in addition to ripgrep.
3. Test that frontmatter-only raw hits do not consume a small result limit.
4. Add actual CRLF end-to-end coverage if surgical.

The embedded current diff is the complete candidate PR after addressing all four:
- source lines drop only the synthetic final split element when content ends in newline;
- source bodies are loaded only for paths present in raw search results;
- a frontmatter-only decoy competes with a real body match under limit 1;
- a real CRLF fixture verifies body exclusion, original line 5, and context.
Focused content/list/search tests pass: 4 files, 235 tests.
The earlier full non-PTY suite passed after the legacy test contract was updated: 111 files, 2,979 tests, 3 skipped.
The PTY suite passed: 94 suites, 176 tests, 1 skipped.
All build, package, type, lint, knip, schema, and docs gates passed before this surgical follow-up.

Review constraints:
- Review only the embedded diff.
- Do not use tools or edit files.
- Verify issue #812 correctness, the four follow-up resolutions, engine parity, line/context behavior, tests, docs, performance, and scope.
- Start with exactly one verdict label: BLOCKERS, NON-BLOCKING, or NO BLOCKERS.
- Separate concrete blockers from optional notes.
...
@@ -101,6 +101,106 @@ interface RgJsonMessage {
   };
 }
...
+
+    await writeFile(
+      join(vaultDir, 'Notes', 'Limit Decoy.md'),
+      `---
+type: note
+status: boundary-token
+---
...
+    );
   ...
   afterAll(async () => {
@@ -136,6 +163,22 @@ All tests have been written and are passing.
     });
   });
...
@@ -176,6 +219,54 @@ All tests have been written and are passing.
...
+
     it('should be case-insensitive by default', async () => {
       const result = await searchContent({
         pattern: 'DEPLOYMENT',
@@ -259,7 +350,7 @@ All tests have been written and are passing.
...
+        pattern: 'deployment',
         ...
         schema: schema,
         limit: 2,
@@ -271,7 +362,7 @@ All tests have been written and are passing.
...
+        pattern: 'deployment',
         ...
         schema: schema,
         limit: 1,
@@ -281,7 +372,7 @@ All tests have been written and are passing.
