You are performing the final follow-up readiness review of an embedded Bowerbird pull request diff.

Task: fix #811 and #813. Pooled ordinary creation must honor the shared output-directory resolver while explicit type/ancestor directories and owned placement retain precedence. Interactive and JSON creation must persist built-in name consistently, preserving note identity separately from filename normalization/pattern paths. Native forks and scaffolding must remain stable.

A prior review had no release blocker but identified these risks. Verify the final diff resolves them:
1. Interactive and JSON template/default identity precedence must agree.
2. Creation tests must cover explicit ancestor-directory precedence.
3. JSON safety must be retained around resolver behavior, or the resolver's totality for explicit/inherited/computed/unknown types must be proven directly.
4. Slash-notation plus computed fallback and owned placement without explicit output_dir need regression coverage. Native fork behavior should be assessed against its actual direct sibling-path implementation, not incorrectly assumed to use write-plan.
5. The docs normalization paragraph must not orphan the colon introducing the JSON example.
6. Non-PTY tests must pin the new system-field order even when PTY tests skip.

Constraints:
- Review only the embedded final diff below.
- Do not use tools.
- Do not edit files.
...
- Review docs against implementation, not prose alone.
- Do not repeat resolved prior notes.
...
diff --git a/tests/ts/commands/new-ownership.test.ts b/tests/ts/commands/new-ownership.test.ts
index f8b8b78..e1da700 100644
...
+
   describe('template flags (JSON mode)', () => {
     it('rejects reserved fork provenance in JSON input', async () => {
       const result = await runCLI(
diff --git a/tests/ts/lib/schema.test.ts b/tests/ts/lib/schema.test.ts
index 1997508..aabf720 100644
