You are performing the final merge-readiness review for Bowerbird issue #812.

This is the actual final diff after the prior exact-model reviews. Verify:
- --body excludes top-of-file YAML frontmatter for filtering and detailed matches.
- original file line numbers and body-only context are preserved for LF and CRLF.
- ripgrep and Node fallback share the restriction semantics.
- source loading is lazy to raw-matched files rather than eager whole-vault reads.
- frontmatter-only raw hits cannot consume a small result limit. The decoy is deliberately named AAA Limit Decoy.md so it sorts before the real Boundary Match.md, and the test asserts the surviving path.
- current help/docs/changelog/agent guidance are coherent and scope remains #812.

Evidence:
- Full non-PTY suite: 111 files, 2,979 passed, 3 skipped.
- PTY suite: 94 suites, 176 passed, 1 skipped.
- Focused final matrix after follow-up: 4 files, 235 passed; final content test: 23 passed.
- Build, verify:pack, typecheck, lint, knip, schema check, docs lint/doctor/check/build passed.
- Built Node 22 CLI disposable-vault story passed.

Constraints:
- Review only the embedded diff; no tools or edits.
- Report concrete merge blockers only as blockers.
- First line exactly BLOCKERS, NON-BLOCKING, or NO BLOCKERS.
- Keep it concise.
...
@@ -176,6 +219,55 @@ All tests have been written and are passing.
