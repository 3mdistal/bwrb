You are reviewing an embedded pull request diff for correctness bugs, missed edge cases, security issues, test gaps, documentation drift, CLI/runtime risk, and release risk.

Task: Fix Bowerbird issue #809 by safely exposing scalar date settings through the config command. The intended boundary is:
- expose date_format and date_granularity through text/JSON list and JSON/interactive edit;
- preserve effective defaults YYYY-MM-DD and day;
- validate via the explicit enum guard and full BwrbSchema parse before any write;
- leave nested calendars to guided authoring issue #790;
- leave advanced mention tuning schema-only;
- explicitly classify every ConfigSchema key in tests so future keys cannot drift or become exposed automatically.

Verification already completed: build, package verification, typecheck, lint, knip, 2,979 non-PTY tests, focused dist tests, 9 config PTY tests, schema parity, docs checks/build, and disposable built-CLI testing.
...
- Separate blockers from non-blocking suggestions. Only correctness, contract, safety, or required test/documentation defects are blockers.
- Review docs against the implementation, not merely prose style.
...
+`date_format` accepts the same pattern strings as the schema, including
+`YYYY-MM-DD`, `MM/DD/YYYY`, `DD/MM/YYYY`, and `DD-MM-YYYY`.
 
 ## Configuration Location
 
@@ -77,6 +83,7 @@ bwrb config list
...
@@ -129,6 +137,10 @@ bwrb config edit open_with --json '"editor"'
 # Set complex value
...
index dc56114..eb8e7fe 100644
--- a/docs/skill/SKILL.md
+++ b/docs/skill/SKILL.md
@@ -160,18 +160,21 @@ bwrb config list
...
@@ -185,6 +205,29 @@ describePty('config command PTY tests', () => {
...
@@ -203,6 +249,67 @@ describe('config command', () => {
...
+
     it('should reject unknown mention_exclude_types without writing', async () => {
       ...
       const result = await runCLI(
@@ -284,6 +391,20 @@ describe('config command', () => {
     });
   });
