...
| `owned-wrong-location` | An [**owned** note](/reference/schema/) — one its owner declares in an `owned` field — has been moved out of its owner's `<owner-dir>/<field>/` subtree (error; **auto-fixable**). Unlike `wrong-directory`, the `--fix` move target is computed from ownership, so it **restores the note under its owner** (`<owner-dir>/<field>/`) rather than stranding it in the owned type's top-level `output_dir`. The issue carries the resolving `ownerPath` |
