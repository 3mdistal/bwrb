...
5. **System fields**: Writes `id` and `name` as bwrb-managed frontmatter fields. The reserved `forked-from` provenance field cannot be supplied through `--json` or template defaults
