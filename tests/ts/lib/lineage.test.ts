...
describe('lineage graph', () => {
  it('collects a branched component with signed depth and stable order', () => {
    const a = note('A.md', A);
    const b = note('B.md', B, A);
    const c = note('C.md', C, B);
    const d = note('D.md', D, B);
    const data = snapshot(d, c, b, a);

    const graph = collectLineage(b, buildLineageMaps(data));

    expect(graph.target).toEqual({ path: 'B.md', id: B });
    expect(graph.nodes.map(({ path, depth, relationship }) => ({ path, depth, relationship })))
      .toEqual([
