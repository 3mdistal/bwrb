...
priorityCommand.command('approve').requiredOption('--json-file <path>', 'Suggestion plan JSON').requiredOption('--approval-id <id>', 'External approval receipt').option('--execute', 'Apply the approved plan').option('--output <format>', 'json').addHelpText('after', '\nDry-run by default; --execute is required to modify task files.').action(async (options, command) => {
  ...
  for (const item of plan.tasks) { const task = byId.get(normalizeNoteId(item.id)); if (!task || task.path !== item.path) errors.push(`unknown or path-mismatched task ${item.id}`); else { if (task.rawRevision !== item.revision) errors.push(`stale raw revision for ${item.path}`); if (task.revision !== item.semanticEvidenceRevision) errors.push(`stale semantic evidence for ${item.path}`); } if (planned.has(normalizeNoteId(item.id))) errors.push(`duplicate plan id ${item.id}`); planned.add(normalizeNoteId(item.id)); if (item.override && !item.reason) errors.push(`override requires a reason for ${item.path}`); validateApprovedFactor(item.importance, 'importance', item.path, errors); validateApprovedFactor(item.excitement, 'excitement', item.path, errors); }
  ...
  for (const item of plan.tasks) { const task = byId.get(normalizeNoteId(item.id))!; const path = join(vaultDir, task.path); const raw = await readFile(path, 'utf8'); assertExpectedRevision(item.revision, raw); const parsed = parseNoteContent(raw); const next: Record<string, unknown> = { ...parsed.frontmatter }; if (item.importance !== undefined) { if (item.importance === null) delete next.importance; else next.importance = item.importance; } if (item.excitement !== undefined) { if (item.excitement === null) delete next.excitement; else next.excitement = item.excitement; } Object.assign(next, { 'priority-rank': item.rank, 'priority-override': item.override === true, 'priority-algorithm': PRIORITY_ALGORITHM, 'priority-as-of': plan.asOf, 'priority-basis-revision': semanticEvidenceRevision(buildNoteContent(next, parsed.body)), 'priority-reviewed': plan.asOf, 'priority-approval-id': options.approvalId }); if (item.override) next['priority-reason'] = item.reason; else delete next['priority-reason']; await writeNote(path, next, parsed.body); }
